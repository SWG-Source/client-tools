/****************************************************************************
** $Id: unixmake2.cpp 2051 2007-02-21 10:04:20Z chehrlic $
**
...
UnixMakefileGenerator::writeMakeParts(QTextStream &t)
{
	...
	t << "TARGETL	= " << var("TARGET_la") << endl;
		...
		if(!project->isEmpty("QMAKE_CYGWIN_SHLIB") ) {
			if (project->first("TEMPLATE") == "lib" && project->isActiveConfig("dll")) {
				if(project->isActiveConfig("plugin") ) {
					t << "TARGET   = cyg" << var("QMAKE_ORIG_TARGET") << "-" << var("VER_MAJ") << "." << var("QMAKE_EXTENSION_SHLIB") << endl;
					t << "TARGET0   = " << "lib" << var("QMAKE_ORIG_TARGET") << ".dll.a"  << endl;
				}
				else {
					t << "TARGET   = ../bin/cyg" << var("QMAKE_ORIG_TARGET") << "-" << var("VER_MAJ") << "." << var("QMAKE_EXTENSION_SHLIB") << endl;
					t << "TARGET0   = " << "lib" << var("QMAKE_ORIG_TARGET") << ".dll.a"  << endl;
					t << "TARGETA   = " << var("TARGETA") << endl;
				}
			}
			else if (project->first("TEMPLATE") == "lib" && project->isActiveConfig("staticlib")) {
				t << "TARGET   = " << "lib" << var("QMAKE_ORIG_TARGET") << ".a" << endl;
			}
	...
	if(!destdir.isEmpty() && !project->isEmpty("QMAKE_CYGWIN_SHLIB") && !project->isActiveConfig("plugin") )
	...
	if(project->isActiveConfig("compile_libtool")) {
        ...
        project->variables()["QMAKE_LINK_SHLIB_CMD"] = "$(LINK) $(LFLAGS) -o $(DESTDIR)$(TARGET) $(OBJECTS) $(OBJMOC) $(LIBS) $(OBJCOMP)";
    t << "\n\t"
      << "-$(DEL_FILE) $(DESTDIR)$(TARGET) $(DESTDIR)$(TARGET0)" << "\n\t"
      << var("QMAKE_LINK_SHLIB_CMD") << "\n\t";
    t << varGlue("QMAKE_LN_SHLIB",""," "," $(TARGET) $(DESTDIR)$(TARGET0)");
    if(!project->isEmpty("QMAKE_POST_LINK"))
        t << "\n\t" << var("QMAKE_POST_LINK");
    t << endl << endl;
...
UnixMakefileGenerator::writeLibtoolFile()
{
    ...
    if (!project->isEmpty("QMAKE_CYGWIN_SHLIB")) {
    	if(project->isActiveConfig("plugin") )
				t << "dlname='cyg" << var("QMAKE_ORIG_TARGET") << "-" << var("VER_MAJ") << "." << var("QMAKE_EXTENSION_SHLIB") << "'\n\n" << endl;
			else
				t << "dlname='../bin/cyg" << var("QMAKE_ORIG_TARGET") << "-" << var("VER_MAJ") << "." << var("QMAKE_EXTENSION_SHLIB") << "'\n\n" << endl;
		}
    else {  
      t << "dlname='" << var(project->isActiveConfig("plugin") ? "TARGET" : "TARGET_x")
      << "'\n\n";
		}
	...
	t << var("TARGET");
			...
			t << "lib" << var("QMAKE_ORIG_TARGET") << ".dll.a";
    }
    else {
    	if (project->isEmpty("QMAKE_HPUX_SHLIB"))
	    t << var("TARGET_x.y.z") << " ";
