/****************************************************************************
** $Id: project.cpp 2051 2007-02-21 10:04:20Z chehrlic $
**
...
QMakeProject::isActiveConfig(const QString &x, bool regex, QMap<QString, QStringList> *place)
{
    ...
    bool crossCompileWin = Option::target_mode == Option::TARG_UNIX_MODE
    	&& spec.find("win32") == 0  ? 1 : 0;

    if (crossCompileWin && Option::target_mode == Option::TARG_UNIX_MODE && x == "win32")
	return TRUE;
    if(!crossCompileWin && Option::target_mode == Option::TARG_UNIX_MODE && x == "unix")
	return TRUE;
    if((Option::target_mode == Option::TARG_MACX_MODE || Option::target_mode == Option::TARG_QNX6_MODE) && x == "unix")
	return TRUE;
