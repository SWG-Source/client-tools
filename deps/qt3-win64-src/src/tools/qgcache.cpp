/****************************************************************************
** $Id: qgcache.cpp 2051 2007-02-21 10:04:20Z chehrlic $
**
...
public:
	...
	{ return QGDict::remove_int((long)(Q_ULONG)item->key,item);}
			    ...
			    int cost, int priority)
{
	...
	dict->insert_int( (long)(Q_ULONG)key, ci );
    tCost += cost;
    return TRUE;
}
...
QPtrCollection::Item QGCache::take_other( const char *key )
{
	...
	ci = dict->take_int( (long)(Q_ULONG)key );
    Item d;
    if ( ci ) {
	d = ci->data;
	tCost -= ci->cost;
...
QPtrCollection::Item QGCache::find_other( const char *key, bool ref ) const
{
    QCacheItem *ci = keytype == AsciiKey ? dict->find_ascii(key)
					 : dict->find_int((long)(Q_ULONG)key);
#if defined(QT_DEBUG)
    lruList->finds++;
#endif
    if ( ci ) {
...
{
    QCacheItem *item = it->current();
    return item ? (long)(Q_ULONG)item->key : 0;
}
