/****************************************************************************
** $Id: qgdict.cpp 2051 2007-02-21 10:04:20Z chehrlic $
**
...
{
    QIntBucket *n;
    int index = (int)((Q_ULONG)key % vlen);	// simple hash
    if ( op == op_find ) {			// find
...
QIntBucket *QGDict::unlink_int( long key, QPtrCollection::Item d )
{
    ...
    QIntBucket *n;
    QIntBucket *prev = 0;
    int index = (int)((Q_ULONG)key % vlen);
...
QDataStream &QGDict::read( QDataStream &s )
{
	...
	switch ( keytype ) {
	    ...
	    case PtrKey:
		{
		    ...
		    // at all, ever?
		    if ( k )
			look_ptr( (void *)(Q_ULONG)k, d, op_insert );
		}
		break;
	}
    }
