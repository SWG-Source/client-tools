/****************************************************************************
** $Id: qscriptengine.cpp 2051 2007-02-21 10:04:20Z chehrlic $
**
...
template <class T>
Q_INLINE_TEMPLATES QVarLengthArray<T>::QVarLengthArray(int asize)
    : s(asize) {
    if (s > Prealloc) {
...
static bool arabic_shape(QShaperItem *item)
{
    ...
    if (item->font->stringToCMap((QChar *)shapedChars.data(), slen, item->glyphs, item->advances,
                                 &item->num_glyphs, item->flags & QTextEngine::RightToLeft) != QFontEngine::NoError)
        return FALSE;
