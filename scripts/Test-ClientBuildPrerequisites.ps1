...
if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
    throw "The build prerequisite manifest is missing: $manifestPath"
}
...
function Test-VisualStudioInstallation {
    ...
    $x64CompilerPattern = Join-Path $normalizedRoot "VC\Tools\MSVC\*\bin\Hostx64\x64\cl.exe"
    $x86CompilerPattern = Join-Path $normalizedRoot "VC\Tools\MSVC\*\bin\Hostx64\x86\cl.exe"
    ...
    if (Test-Path -LiteralPath $vcTargetsRoot -PathType Container) {
        $toolsetFiles = @(
            Get-ChildItem -LiteralPath $vcTargetsRoot -Recurse -Filter "Toolset.props" -File -ErrorAction SilentlyContinue |
                Where-Object {
                    $_.FullName.EndsWith("Platforms\x64\PlatformToolsets\$PlatformToolset\Toolset.props", [StringComparison]::OrdinalIgnoreCase) -or
                    $_.FullName.EndsWith("Platforms\Win32\PlatformToolsets\$PlatformToolset\Toolset.props", [StringComparison]::OrdinalIgnoreCase)
                }
        )
    }

    $x64Compilers = @(Get-ChildItem -Path $x64CompilerPattern -File -ErrorAction SilentlyContinue | Sort-Object FullName -Descending)
    $x86Compilers = @(Get-ChildItem -Path $x86CompilerPattern -File -ErrorAction SilentlyContinue | Sort-Object FullName -Descending)
    $x64Compiler = $null
    $x86Compiler = $null
    if ($x64Compilers.Count -gt 0) {
        $x64Compiler = $x64Compilers[0].FullName
    }
    if ($x86Compilers.Count -gt 0) {
        $x86Compiler = $x86Compilers[0].FullName
    }

    $compilerDirectory = if ($x64Compiler) { Split-Path -Parent $x64Compiler } else { $null }
    $librarian = $null
    $nmake = $null
    ...
    [pscustomobject]@{
        ...
        CompilerPath = $x64Compiler
        X86CompilerPath = $x86Compiler
...
if (-not $Quiet) {
    Write-Host "Client build prerequisite profile: $($status.Profile)"
    ...
    Write-PrerequisiteStatus -Ready $vendoredInputs.Ready -Name "Vendored client dependencies"
}
...
if (-not $AllowMissing -and -not $status.Ready) {
    throw ("Client build prerequisites are incomplete: {0}. Install the missing components and run this check again." -f ($missing -join "; "))
}
