...
{
public:
	SearchCondition() : attributeNameCrc(), requiredAttribute(false), comparison(SCC_int), intMin(0), intMax(0), floatMin(0.0f), floatMax(0.0f), stringValue()
	{
	}

	SearchCondition(SearchCondition const &rhs) : attributeNameCrc(rhs.attributeNameCrc), requiredAttribute(rhs.requiredAttribute), comparison(rhs.comparison), intMin(rhs.intMin), intMax(rhs.intMax), floatMin(rhs.floatMin), floatMax(rhs.floatMax), stringValue(rhs.stringValue)
	{
	}

	explicit SearchCondition(uint32 const pAttributeNameCrc, bool const pRequiredAttribute, int const pIntMin, int const pIntMax) : attributeNameCrc(pAttributeNameCrc), requiredAttribute(pRequiredAttribute), comparison(SCC_int), intMin(pIntMin), intMax(pIntMax), floatMin(0.0f), floatMax(0.0f), stringValue()
	{
	}

	explicit SearchCondition(uint32 const pAttributeNameCrc, bool const pRequiredAttribute, double const pFloatMin, double const pFloatMax) : attributeNameCrc(pAttributeNameCrc), requiredAttribute(pRequiredAttribute), comparison(SCC_float), intMin(0), intMax(0), floatMin(pFloatMin), floatMax(pFloatMax), stringValue()
	{
	}

	explicit SearchCondition(uint32 const pAttributeNameCrc, bool const pRequiredAttribute, SearchConditionComparison const pComparison, std::string const &pStringValue) : attributeNameCrc(pAttributeNameCrc), requiredAttribute(pRequiredAttribute), comparison(pComparison), intMin(0), intMax(0), floatMin(0.0f), floatMax(0.0f), stringValue(pStringValue)
	{
	}
