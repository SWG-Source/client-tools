...
Range IntersectLineSphere ( Line3d const & L, Sphere const & S )
{
	...
	float radi2 = S.getRadius() * S.getRadius();

	if(Q.solveFor(radi2,min,max))
	{
...
Range IntersectPointSphere ( Vector const & P, Vector const & V, Sphere const & S )
{
	...
	real radi2 = S.getRadius() * S.getRadius();

	float newMin;
    float newMax;

	if(Q.solveFor( radi2, newMin, newMax ))
	{
...
Range IntersectSphereSphere ( Sphere const & A, Vector const & velocity, Sphere const & B )
{
	...
	real sumradi2 = sumRadius * sumRadius;

	float newMin;
    float newMax;

	if(Q.solveFor( sumradi2, newMin, newMax ))
	{
		return Range(newMin,newMax);
	}
	else
