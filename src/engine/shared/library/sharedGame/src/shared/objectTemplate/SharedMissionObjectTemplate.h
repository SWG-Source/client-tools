...
#ifndef _INCLUDED_SharedMissionObjectTemplate_H
...
//@BEGIN TFD ID
	enum
	{
		SharedMissionObjectTemplate_tag = TAG(S,M,S,O)
	};
//@END TFD ID
public:
	         SharedMissionObjectTemplate(const std::string & filename);
	virtual ~SharedMissionObjectTemplate();
...
private:
	...
	// no copying
	SharedMissionObjectTemplate(const SharedMissionObjectTemplate &);
	SharedMissionObjectTemplate & operator =(const SharedMissionObjectTemplate &);
};


inline void SharedMissionObjectTemplate::install(bool allowDefaultTemplateParams)
{
...
#endif	// _INCLUDED_SharedMissionObjectTemplate_H
