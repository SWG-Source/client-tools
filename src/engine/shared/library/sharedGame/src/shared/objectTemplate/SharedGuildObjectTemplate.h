...
#ifndef _INCLUDED_SharedGuildObjectTemplate_H
...
//@BEGIN TFD ID
	enum
	{
		SharedGuildObjectTemplate_tag = TAG(S,G,L,D)
	};
//@END TFD ID
public:
	         SharedGuildObjectTemplate(const std::string & filename);
	virtual ~SharedGuildObjectTemplate();
...
private:
	...
	// no copying
	SharedGuildObjectTemplate(const SharedGuildObjectTemplate &);
	SharedGuildObjectTemplate & operator =(const SharedGuildObjectTemplate &);
};


inline void SharedGuildObjectTemplate::install(bool allowDefaultTemplateParams)
{
...
#endif	// _INCLUDED_SharedGuildObjectTemplate_H
