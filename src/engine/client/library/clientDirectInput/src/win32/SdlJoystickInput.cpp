...
namespace SdlJoystickInputNamespace
{
	...
	int const   cms_maxAxes        = 6;   // X,Y,Z,RX,RY,RZ
	int const   cms_maxSliders     = 2;
	int const   cms_maxButtons     = 32;
	int const   cms_maxHats        = 4;
	...
	float const cms_axisDeadZone   = 0.10f;
	float const cms_axisSaturation = 0.90f;
	...
	{
		float v = (static_cast<float>(raw) + 32768.0f) / 65535.0f;
		if (v < 0.0f) v = 0.0f;
		if (v > 1.0f) v = 1.0f;
		return v;
	}
	...
	float hatToAngle(int hat)
	{
		...
		switch (hat)
		{
		case SDL_HAT_UP:        return 0.0f;
		case SDL_HAT_RIGHTUP:   return pi * 0.25f;
		case SDL_HAT_RIGHT:     return pi * 0.50f;
		case SDL_HAT_RIGHTDOWN: return pi * 0.75f;
		case SDL_HAT_DOWN:      return pi;
		case SDL_HAT_LEFTDOWN:  return pi * 1.25f;
		case SDL_HAT_LEFT:      return pi * 1.50f;
		case SDL_HAT_LEFTUP:    return pi * 1.75f;
		default:                return -1.0f;  // SDL_HAT_CENTERED
		}
	}
	...
	void emitAxis(int number, int axisIndex, float value)
	{
		switch (axisIndex)
		{
		case 0: IoWinManager::queueJoystickTranslateX(number, value); break;
		case 1: IoWinManager::queueJoystickTranslateY(number, value); break;
		case 2: IoWinManager::queueJoystickTranslateZ(number, value); break;
		case 3: IoWinManager::queueJoystickRotateX(number, value);    break;
		case 4: IoWinManager::queueJoystickRotateY(number, value);    break;
		case 5: IoWinManager::queueJoystickRotateZ(number, value);    break;
		default: break;
		}
	}
	...
	int dpadToHat(SDL_Gamepad * pad)
	{
		...
		if (SDL_GetGamepadButton(pad, SDL_GAMEPAD_BUTTON_DPAD_UP))    hat |= SDL_HAT_UP;
		if (SDL_GetGamepadButton(pad, SDL_GAMEPAD_BUTTON_DPAD_DOWN))  hat |= SDL_HAT_DOWN;
		if (SDL_GetGamepadButton(pad, SDL_GAMEPAD_BUTTON_DPAD_LEFT))  hat |= SDL_HAT_LEFT;
		if (SDL_GetGamepadButton(pad, SDL_GAMEPAD_BUTTON_DPAD_RIGHT)) hat |= SDL_HAT_RIGHT;
		return hat;
	}

	bool isDpadButton(int gamepadButton)
	{
		return gamepadButton == SDL_GAMEPAD_BUTTON_DPAD_UP
		    || gamepadButton == SDL_GAMEPAD_BUTTON_DPAD_DOWN
		    || gamepadButton == SDL_GAMEPAD_BUTTON_DPAD_LEFT
		    || gamepadButton == SDL_GAMEPAD_BUTTON_DPAD_RIGHT;
	}
