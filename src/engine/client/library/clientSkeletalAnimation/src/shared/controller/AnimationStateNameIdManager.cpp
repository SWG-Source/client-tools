...
void AnimationStateNameIdManager::install()
{
	...
	const int rootId = ms_stateNameList.submitName(std::shared_ptr<CrcLowerString>(new ConstCharCrcLowerString("root")));
...
AnimationStateNameId AnimationStateNameIdManager::createId(const CrcLowerString &stateName)
{
	...
	return AnimationStateNameId(ms_stateNameList.submitName(std::shared_ptr<CrcLowerString>(new CrcLowerString(stateName))));
}
