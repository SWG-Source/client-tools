...
{
	constexpr int cs_resourceNameBytes = 96;
	constexpr unsigned int cs_constantRegisterBytes = 16;
...
void Direct3d11_ShaderReflectionNamespace::checkConstantBuffer(ID3D11ShaderReflection *reflection, char const *name, bool isVertexShader, Direct3d11_ShaderReflection::Result &result)
{
	...
	for (UINT i = 0; i < bufferDescription.Variables; ++i)
	{
		...
		for (int t = 0; t < tableCount; ++t)
			...
			{
				unsigned int const expected = static_cast<unsigned int>(table[t].registerIndex) * cs_constantRegisterBytes;
...
void Direct3d11_ShaderReflectionNamespace::buildSamplerMap(ID3D11ShaderReflection *reflection, char const *name, Direct3d11_ShaderReflection::Result &result)
{
	...
	char samplerNames[Direct3d11_ShaderReflection::MAX_SAMPLER_SLOTS][cs_resourceNameBytes];
	char textureNames[Direct3d11_ShaderReflection::MAX_SAMPLER_SLOTS][cs_resourceNameBytes];
	...
	for (UINT i = 0; i < shaderDescription.BoundResources; ++i)
	{
		...
		if (bindDescription.Type == D3D_SIT_SAMPLER)
		{
			...
			{
				strncpy(samplerNames[samplerCount], bindDescription.Name ? bindDescription.Name : "", cs_resourceNameBytes - 1);
				samplerNames[samplerCount][cs_resourceNameBytes - 1] = 0;
		...
		else if (bindDescription.Type == D3D_SIT_TEXTURE)
		{
			...
			{
				strncpy(textureNames[textureCount], bindDescription.Name ? bindDescription.Name : "", cs_resourceNameBytes - 1);
				textureNames[textureCount][cs_resourceNameBytes - 1] = 0;
...
bool Direct3d11_ShaderReflection::reflect(ID3DBlob *bytecode, char const *name, bool isVertexShader, Result &result)
{
	...
	if (!result.checkedConstantCount && !ms_warnedAboutVacuousGuard)
	{
		...
		WARNING(true, ("Direct3d11: '%s' declared %d constant(s) in $Globals and NONE of them matched a known name, so the constant ABI guard checked nothing. The name table in Direct3d11_ShaderReflection.cpp does not match what the shipped .inc files declare and must be corrected.", name, result.constantExtentBytes / cs_constantRegisterBytes));
	}
