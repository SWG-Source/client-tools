...
namespace
{
	constexpr double cs_millisecondsPerSecond = 1000.0;
	constexpr long long cs_microsecondsPerSecond = 1000000;
} // namespace
...
double Direct3d11_Metrics::ticksToMilliseconds(long long ticks)
{
	...
	return static_cast<double>(ticks) * cs_millisecondsPerSecond / static_cast<double>(frequency.QuadPart);
}
...
int Direct3d11_MetricsNamespace::elapsedMicroseconds(long long from, long long to)
{
	...
	return static_cast<int>(((to - from) * cs_microsecondsPerSecond) / frequency.QuadPart);
}
...
void Direct3d11_Metrics::reportHitches()
{
	...
	{
		double const milliseconds = static_cast<double>(now.QuadPart - previous.QuadPart) * cs_millisecondsPerSecond / static_cast<double>(frequency.QuadPart);
		...
		// window says which, and carries the average split so the answer is not just "slow".
		{
			...
			windowScene += static_cast<double>(sceneMicroseconds) / cs_millisecondsPerSecond;
			windowPresent += static_cast<double>(presentMicroseconds) / cs_millisecondsPerSecond;
			windowBeforePresent += static_cast<double>(beforePresentMicroseconds) / cs_millisecondsPerSecond;
			windowAfterPresent += static_cast<double>(afterPresentMicroseconds) / cs_millisecondsPerSecond;

			if (milliseconds < 17.0)
				++windowUnder17;
			else if (milliseconds < 25.0)
				++windowUnder25;
			else if (milliseconds < 34.0)
				++windowUnder34;
			...
			if (windowFrames >= cms_windowFrames)
			{
				...
				WARNING(true, ("Direct3d11 FRAMES: last %d frames averaged %.1f ms (%.0f fps), worst %.1f ms; distribution <17 ms %d, <25 ms %d, <34 ms %d, <50 ms %d, >=50 ms %d; average %.1f ms in scene, %.1f ms engine before Present, %.1f ms in Present, %.1f ms engine after Present; GPU recently %.1f ms; %d draw(s) last frame; last frame had %d lit batch(es), highest scene ambient %.3f",
							   windowFrames, averageMilliseconds, (averageMilliseconds > 0.0) ? (cs_millisecondsPerSecond / averageMilliseconds) : 0.0, windowWorst,
							   ...
							   litBatches, static_cast<double>(maxAmbientMilli) / cs_millisecondsPerSecond));
		...
		if (milliseconds > 40.0 && reportsRemaining > 0)
		{
			...
			double const sceneMilliseconds = static_cast<double>(sceneMicroseconds) / cs_millisecondsPerSecond;
			double const presentMilliseconds = static_cast<double>(presentMicroseconds) / cs_millisecondsPerSecond;
			double const beforePresentMilliseconds = static_cast<double>(beforePresentMicroseconds) / cs_millisecondsPerSecond;
			double const afterPresentMilliseconds = static_cast<double>(afterPresentMicroseconds) / cs_millisecondsPerSecond;
			...
			WARNING(true, ("Direct3d11 HITCH: frame %d took %.1f ms -- %.1f ms in scene, %.1f ms engine before Present, %.1f ms in Present, %.1f ms engine after Present, %.1f ms unaccounted; GPU recently %.1f ms; at most %d light(s) in any batch, %d of %d batch(es) lit; %d draw(s) spent %.1f ms in prepareToDraw, %.1f ms in Draw itself, %.1f ms binding shaders; %d constant upload(s) of %d byte(s); ring %d discard(s) and %d append(s) of %d byte(s) taking %.1f ms; bind misses layout %d vb %d ib %d vs %d ps %d blend %d depth %d rast %d samp %d srv %d; streamed in %d vertex buffer(s), %d index buffer(s), %d texture(s) taking %.1f ms (%.1f creating buffers, %.1f creating textures, %.1f uploading mips: %.1f zeroing, %.1f converting, %.1f in UpdateSubresource); created %d state object(s), %d input layout(s), %d constant buffer(s); compiled %d shader(s) in %.1f ms; %d blocking staging map(s), %d bake readback(s), %d render target switch(es), %d draw(s)",
						   ...
						   shaderCompiles, static_cast<double>(shaderCompileMicroseconds) / cs_millisecondsPerSecond,
...
void Direct3d11_Metrics::report()
{
	...
	WARNING(ms_runShaderCompiles != 0, ("Direct3d11 metrics: shader compilation cost %.1f ms over the run for %d program(s), %.1f ms average; the peak frame spent %.1f ms in D3DCompile.",
										static_cast<double>(ms_runCompileMicroseconds) / cs_millisecondsPerSecond,
										ms_runShaderCompiles,
										static_cast<double>(ms_runCompileMicroseconds) / (cs_millisecondsPerSecond * static_cast<double>(ms_runShaderCompiles)),
										static_cast<double>(ms_peakInFrameCompileMicroseconds) / cs_millisecondsPerSecond));
