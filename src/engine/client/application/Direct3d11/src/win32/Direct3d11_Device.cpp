...
{
	constexpr int cs_debugNameBytes = 256;
	constexpr int cs_hresultDescriptionBytes = 32;
	constexpr int cs_adapterDescriptionBytes = 128;
	constexpr unsigned long long cs_bytesPerMegabyte = 1024ULL * 1024ULL;
	constexpr int cs_minimumExpectedVideoMemoryMegabytes = 100;
...
char const *Direct3d11_Device::describeHresult(HRESULT hresult)
{
	...
	static char buffer[cs_hresultDescriptionBytes];
...
void Direct3d11_DeviceNamespace::selectAdapter()
{
	...
	char description[cs_adapterDescriptionBytes];
...
void Direct3d11_DeviceNamespace::createDevice()
{
	...
	{
		case ConfigDirect3d11::DT_warp:
			driverType = D3D_DRIVER_TYPE_WARP;
			break;
		case ConfigDirect3d11::DT_reference:
...
void Direct3d11_DeviceNamespace::queryCapabilities()
{
	...
	ms_videoMemoryInMegabytes = static_cast<int>(usable / cs_bytesPerMegabyte);

	WARNING(ms_videoMemoryInMegabytes < cs_minimumExpectedVideoMemoryMegabytes, ("Direct3d11: only %d MB of video memory is reported, so SetupClientGraphics will disable POST and HEAT (and DOT3 below 40 MB).", ms_videoMemoryInMegabytes));

	bool const verbose = ConfigSharedFoundation::getVerboseHardwareLogging();
	REPORT_LOG(verbose, ("Direct3d11 video memory: %d MB dedicated, %d MB shared\n", static_cast<int>(dedicated / cs_bytesPerMegabyte), static_cast<int>(shared / cs_bytesPerMegabyte)));
