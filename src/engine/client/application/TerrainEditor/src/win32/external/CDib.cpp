...
CDib::CDib(CSize size, int nBitCount)
{
        ...
        m_lpBMIH->biPlanes = 1;
        m_lpBMIH->biBitCount = static_cast<uint16> (nBitCount);
...
//	DWORD dwErr = ::GetLastError();
	...
	AttachMemory((LPBYTE) lpvFile + sizeof(BITMAPFILEHEADER), false);
	m_lpvFile = lpvFile;
	m_hFile = hFile;
	m_hMap = hMap;
	return TRUE;
...
BOOL CDib::MakePalette()
{
	...
	pLogPal->palNumEntries = static_cast<uint16> (m_nColorTableEntries);
