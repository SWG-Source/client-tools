...
DialogProgress::DialogProgress(UINT nCaptionID)
{
...
BOOL DialogProgress::DestroyWindow()
{
    ReEnableParent();
    return CDialog::DestroyWindow();
}

void DialogProgress::ReEnableParent()
{
...
BOOL DialogProgress::Create(CWnd *pParent)
{
    ...
    if(!CDialog::Create(DialogProgress::IDD,pParent))
    {
      ReEnableParent();
      return FALSE;
    }
...
void DialogProgress::SetStatus(LPCTSTR lpszMessage)
{
...
void DialogProgress::OnCancel()
{
    m_bCancel=TRUE;
}

void DialogProgress::SetRange(int nLower,int nUpper)
{
    m_nLower = nLower;
    m_nUpper = nUpper;
    m_Progress.SetRange(static_cast<short> (nLower),static_cast<short> (nUpper));
}
  
int DialogProgress::SetPos(int nPos)
{
    PumpMessages();
...
int DialogProgress::SetStep(int nStep)
{
    m_nStep = nStep; // Store for later use in calculating percentage
    return m_Progress.SetStep(nStep);
}

int DialogProgress::OffsetPos(int nPos)
{
    PumpMessages();
...
void DialogProgress::PumpMessages()
{
...
BOOL DialogProgress::CheckCancelButton()
{
...
void DialogProgress::UpdatePercent(int nNewPos)
{
...
{		
	...
	VERIFY(strCaption.LoadString(IDS_BTN_CANCEL));
	m_cancelButton.SetWindowText(strCaption);

    m_Progress.SetRange(static_cast<short> (m_nLower),static_cast<short> (m_nUpper));
