...
#include "ServerTemplateListView.moc"
...
ServerTemplateListView::ServerTemplateListView(QWidget* theParent, const char* theName)
 ...
 *
 */
QDragObject* ServerTemplateListView::dragObject()
{
	return new QTextDrag(ObjectTemplateData::DragMessages::SERVER_TEMPLATE_DRAGGED, this, "menu");
}
 ...
 *
 */
void ServerTemplateListView::populateTemplateTree(const char* name, const QPixmap* pix, const QPixmap* folderPix, QListView* parent, const AbstractFilesystemTree* afst) const
{
	if(afst)
	{
 ...
 *
 */
const std::string ServerTemplateListView::constructRelativePath(const QListViewItem* item, bool& isLeaf, bool& isNew, bool& isEdit) const
{
	std::string result;
...
void ServerTemplateListView::onSelectionChanged() const
{
 ...
 *
 */
void ServerTemplateListView::onRefreshList()
{
	clear();
 ...
 */
void ServerTemplateListView::onContextMenuRequested(QListViewItem* item, const QPoint& p, int)
{
	if(item == 0)
		return;
	...
	IGNORE_RETURN(aot->m_serverAddToFavorites->addTo(m_pop));
