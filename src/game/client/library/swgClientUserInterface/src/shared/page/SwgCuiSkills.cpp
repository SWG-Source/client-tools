	  ...
	  m_selectedSkill()
{
	...
	if (m_pageProfession)
	{
		...
		if (infoModsNameObj)
			m_dsInfoModsName = static_cast<UIDataSource *>(infoModsNameObj);
		if (infoModsPointsObj)
			m_dsInfoModsPoints = static_cast<UIDataSource *>(infoModsPointsObj);
		if (infoCmdsNameObj)
			m_dsInfoCmdsName = static_cast<UIDataSource *>(infoCmdsNameObj);
		if (infoCmdsIconsObj)
			m_dsInfoCmdsIcons = static_cast<UIDataSource *>(infoCmdsIconsObj);
		...
		if (spTextObj)
			m_textSkillPoints = static_cast<UIText *>(spTextObj);
		if (spBtnObj)
			m_buttonSurrender = static_cast<UIButton *>(spBtnObj);
		...
		if (graphsObj)
			m_pageGraphs = static_cast<UIPage *>(graphsObj);
		if (graph4x4Obj)
			m_pageGraph4x4 = static_cast<UIPage *>(graph4x4Obj);
		if (graph2x4Obj)
			m_pageGraph2x4 = static_cast<UIPage *>(graph2x4Obj);
		if (graph1x4Obj)
			m_pageGraph1x4 = static_cast<UIPage *>(graph1x4Obj);
		if (graphPyrObj)
			m_pageGraphPyramid = static_cast<UIPage *>(graphPyrObj);
...
{
	if (m_pageGraph4x4)
		m_pageGraph4x4->SetVisible(false);
	if (m_pageGraph2x4)
		m_pageGraph2x4->SetVisible(false);
	if (m_pageGraph1x4)
		m_pageGraph1x4->SetVisible(false);
	if (m_pageGraphPyramid)
		m_pageGraphPyramid->SetVisible(false);
}
								   ...
								   std::string const &skillName, bool hasSkill, bool nextTrainable)
{
	...
	if (xpbar && fill)
	{
		...
		if (fillH < 2 && ratio > 0.0f)
			fillH = 2; // keep a sliver visible
		if (fillH > boxH)
			fillH = boxH;
...
void SwgCuiSkills::populateSelectedSkill()
{
	...
	if (m_dsInfoModsName)
		m_dsInfoModsName->Clear();
	if (m_dsInfoModsPoints)
		m_dsInfoModsPoints->Clear();
	if (m_dsInfoCmdsName)
		m_dsInfoCmdsName->Clear();
	if (m_dsInfoCmdsIcons)
		m_dsInfoCmdsIcons->Clear();
