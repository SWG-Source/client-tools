...
#ifndef INCLUDED_UnicodeBlocks_H
	...
	namespace Blocks
	{
		...
		*/
		struct Data 
		{
			...
			bool                     addRangeGroup (bool inclusive, const Unicode::NarrowString & str);
		...
		*/
		...
		public:
			...
			typedef std::map <Unicode::NarrowString, Data> NameMap_t;
			
			const Data &           findBlock (Id id) const;
			Data &                 findBlock (Id id);
			const Data &           findBlock (const Unicode::NarrowString &) const;
		...
		*/
		...
		inline const Data & Mapping::findBlock (const Unicode::NarrowString & name) const
		{
