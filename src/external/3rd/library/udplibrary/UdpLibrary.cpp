...
UdpManager::UdpManager(const UdpManager::Params *params)
{
	...
	if (wolfSSL_CTX_load_verify_locations(this->ctx, caCertLoc, 0) != SSL_SUCCESS) {
	...
	printf("Started ssl...so here's hoping this mofo works");
}
...
void UdpManager::CreateAndBindSocket(int usePort)
{
	...
	{
		wolfSSL_set_fd(this->ssl, mUdpSocket);

		/*int on = 1; // copypasta from wolf for now

		if (setsockopt(mUdpSocket, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) < 0) {
		...
		wolfSSL_dtls_set_peer(this->ssl, &addr_loc, sizeof(addr_loc));
...
UdpManager::PacketHistoryEntry *UdpManager::ActualReceive()
{
	...
	if (res != SOCKET_ERROR)
	{
		...
		/*if (connect(res, (const struct sockaddr *) &addr_from.sin_addr.s_addr, sizeof(addr_from.sin_addr.s_addr)) != 0) {
		printf("Udp connect failed.\n");
		return nullptr;
		}*/

		if (wolfSSL_accept(this->ssl) != SSL_SUCCESS) {
			int e = wolfSSL_get_error(this->ssl, 0);
			printf("error = %d, %s\n", e, wolfSSL_ERR_reason_error_string(e));
			printf("SSL_accept failed.\n");

		}

		int recvLen = wolfSSL_read(this->ssl, mPacketHistory[pos]->mBuffer, sizeof(mPacketHistory[pos]->mBuffer) - 1);

		if (recvLen > 0) {
			printf("heard %d bytes\n", recvLen);
			printf("I heard this: \"%s\"\n", mPacketHistory[pos]->mBuffer);
		}
		else if (recvLen < 0) {
			int readErr = wolfSSL_get_error(this->ssl, 0);
...
void UdpManager::ActualSendHelper(const uchar *data, int dataLen, UdpIpAddress ip, int port)
{
	if (mParams.simulateOutgoingLossPercent > 0 && ((rand() % 100) < mParams.simulateOutgoingLossPercent))
	 ...
	 printf("Trying to bind socket to %s %p");
	 wolfSSL_dtls_set_peer(this->ssl, &addr_dest, sizeof(addr_dest));

	 printf("Done.");

	printf("Trying to send, here's hoping...");

	if (wolfSSL_connect(this->ssl) != SSL_SUCCESS) {
		int err1 = wolfSSL_get_error(this->ssl, 0);
		printf("err = %d, %s\n", err1, wolfSSL_ERR_reason_error_string(err1));
		printf("SSL_connect failed");
	}

	if (wolfSSL_write(this->ssl, (const char *)data, dataLen) < 0) {
...
void UdpConnection::ProcessRawPacket(const UdpManager::PacketHistoryEntry *e) {
	...
	if (IsNonEncryptPacket(e->mBuffer)) {
		...
		if (mConnectionConfig.crcBytes > 0) {
			...
			int actualCrc = UdpMisc::Crc32(finalStart,
				finalLen - mConnectionConfig.crcBytes, mConnectionConfig.encryptCode);
			int wantCrc = 0;
