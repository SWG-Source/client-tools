            ...
            ios_base::openmode __mode)
{
  ...
  else if ((_in_  && (!(_M_mode & ios_base::in) || this->gptr() == 0)) ||
           (_out_ && (!(_M_mode & ios_base::out) || this->pptr() == 0)))
    return pos_type(off_type(-1));
  ...
  case ios_base::end:
    ...
    break;
  case ios_base::cur:
    __newoff = _in_ ? this->gptr() - this->eback() 
...
{
  bool _in_  = (__mode & ios_base::in) != 0;
  bool _out_ = (__mode & ios_base::out) != 0;

  if ((_in_  && (!(_M_mode & ios_base::in) || this->gptr() == 0)) ||
      (_out_ && (!(_M_mode & ios_base::out) || this->pptr() == 0)))
    return pos_type(off_type(-1));
