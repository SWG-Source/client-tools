...
_OutputIter  _STLP_CALL
__put_float(char* __ibuf, char* __iend, _OutputIter _out_,
            ...
            wchar_t __sep, const string& __grouping)
{
  ...
  return __copy_float_and_fill(__wbuf, __eend, _out_,
            ...
            char __sep, const string& __grouping)
{
  ...
  return __copy_float_and_fill(__ibuf, __iend, _out_,
                               __f.flags(), __f.width(0), __fill, '+', '-');
}
...
{
  if (__len >= __wid)
    return copy(__buf, __buf + __len, _out_);
  else {
    ptrdiff_t __pad = __wid - __len;
    ios_base::fmtflags __dir = __flg & ios_base::adjustfield;

    if (__dir == ios_base::left) {
      _out_ = copy(__buf, __buf + __len, _out_);
      return fill_n(_out_, __pad, __fill);
    }
      ...
      *_out_++ = __buf[0];
      _out_ = fill_n(_out_, __pad, __fill);
      return copy(__buf + 1, __buf + __len, _out_);
    }
      ...
      *_out_++ = __buf[1];
      _out_ = fill_n(_out_, __pad, __fill);
      return copy(__buf + 2, __buf + __len, _out_);
    }
    else {
      _out_ = fill_n(_out_, __pad, __fill);
      return copy(__buf, __buf + __len, _out_);
