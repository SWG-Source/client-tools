...
BEGIN_MESSAGE_MAP(CDxtexDoc, CDocument)
	...
	ON_COMMAND(ID_FORMAT_GENERATESPECULARLOOKUP, OnFormatGeneratespecularlookup)
	ON_UPDATE_COMMAND_UI(ID_FORMAT_GENERATESPECULARLOOKUP, OnUpdateFormatGeneratespecularlookup)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
struct GenerateSpecularLookupData
{
	float power;
};

VOID WINAPI SpecularColorFill(D3DXVECTOR4* pOut, const D3DXVECTOR2* pTexCoord, const D3DXVECTOR2* pTexelSize, LPVOID pData)
{
	float d = pTexCoord->x;
	float s = static_cast<float>(pow(pTexCoord->y, reinterpret_cast<GenerateSpecularLookupData*>(pData)->power));

	if (pTexCoord->x < pTexelSize->x)
		s = 0.0f;
...
void CDxtexDoc::OnFormatGeneratespecularlookup() 
{
	if (IsVolumeMap())
		return;
	if (IsCubeMap())
  ...
  CSpecularPowerDialog specularPowerDialog;
  if( IDOK == specularPowerDialog.DoModal() )
	{
		GenerateSpecularLookupData specularLookupData;
		specularLookupData.power = static_cast<float>(atof(specularPowerDialog.m_specularPower));

		// TODO: Add your command handler code here
		HRESULT hresult = D3DXFillTexture((LPDIRECT3DTEXTURE9)m_ptexOrig, SpecularColorFill, &specularLookupData);
...
void CDxtexDoc::OnUpdateFormatGeneratespecularlookup(CCmdUI* pCmdUI) 
{
