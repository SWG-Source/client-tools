...
BEGIN_MESSAGE_MAP(CDxtexView, CScrollView)
    ...
    ON_COMMAND(ID_VIEW_RGBONLY, OnViewRgbOnly)
    ...
    ON_UPDATE_COMMAND_UI(ID_VIEW_RGBONLY, OnUpdateViewRgbOnly)
...
HRESULT CDxtexView::RenderScene(VOID)
{
    ...
    hr = pd3ddev->SetRenderState(D3DRS_ALPHABLENDENABLE, m_bRgbOnly ? FALSE : TRUE);
...
void CDxtexView::OnViewRgbOnly(VOID)
{
    m_bRgbOnly = !m_bRgbOnly;
    BuildViewSurface(m_bViewOrig, m_CubeFaceCur, m_lwSliceCur, m_lwMipCur, false);
    RenderScene();
    Invalidate(); // force redraw of this view
    m_bTitleModsChanged = TRUE; // force title bar update
}
...
void CDxtexView::OnUpdateViewRgbOnly(CCmdUI* pCmdUI)
{
    pCmdUI->Enable(TRUE);
    pCmdUI->SetCheck(m_bRgbOnly);
}
